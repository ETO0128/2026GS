...
| Exponential weights can be used for time-varying quantile forecasts | Taylor (2007) [20] | 论文提出指数加权分位数回归，并将其解释为对累积分布函数的指数平滑，以处理位置、方差和形状随时间变化的分布。 | **正式方案采用：** 候选残差权重取“曲线相似度 × 时间指数衰减”，再计算逐时加权经验分位数。 | 研究对象为零售需求，不直接确定微网数据的衰减系数；本题的 λ 仅由 1 月 15--31 日的因果滚动验证选择。 |
